...
namespace ScheduleStopwatch
{
    ...
    class VehicleWindowHelper
    {
        ...
        private static void VehicleWindowScheduleTabSeparatorView_Initialize_pof(VehicleWindowScheduleTabSeparatorView __instance, VehicleWindowScheduleTab scheduleTab, int? insertIndex)
        {
            ...
            {
                TaskTimeTemplate = UnityEngine.Object.Instantiate<Transform>(__instance.transform.Find("AddStop"));
            ...
            if (!scheduleTab.EditMode)
            {
                if (insertIndex.HasValue)
                {
                    var settings = Settings.Current;
                    var task = scheduleTab.Vehicle.Schedule.GetTasks()[insertIndex.Value];
                    if (insertIndex.Value == 0 && settings.ShowScheduleTotalTime)
                    {
                        var transform = UnityEngine.Object.Instantiate<Transform>(TotalTimeTemplate, __instance.transform);
                    ...
                    {
                        var transform = UnityEngine.Object.Instantiate<Transform>(TaskTimeTemplate, __instance.transform);
